...
flowchart TD
  ...
  T2 -->|"открытый вопрос: Q3 формат вывода"| T3["T: агенты: краевые случаи парсера + код-ревью"]:::done
  T3 -->|"ревью: 11 находок — 1 high, 6 medium, 4 low; краевые: 10, новое — #quot;"| Q5["Q: что чинить из находок ревью"]:::done
  Q5 -->|"цикл X-A-B: memo даёт 4 вместо 6; RecursionError на 1200 узлах"| H7["H: чинить всё: цепочка без рекурсии, SKIP по словам, стрелки в кавычках, stderr для мусора"]:::done
  Q5 -->|"реальные графы — DAG, баг не проявляется"| H8["H: оставить как есть"]:::rejected
  H7 -->|"Ran 27 tests: OK; push ba8ccd4"| A2["A: mmdstats.py + test_mmdstats.py"]:::done
```
